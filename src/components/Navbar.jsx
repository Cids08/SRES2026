...
export default function Navbar() {
    ...
    return (
        <>
            {/* Fixed header — taken out of document flow so nothing jumps */}
            <header className="fixed top-0 left-0 right-0 z-50">

                {/* Gold stripe */}
                <div className="h-[5px] bg-gradient-to-r from-yellow-700 via-yellow-400 to-yellow-700" />

                {/* Top bar — slides up on scroll using transform, not max-h, to avoid layout shift */}
                <div
                    className="bg-[#0a1f52] transition-all duration-300 origin-top"
                    style={{
                ...
                <nav className="bg-[#0d2460] border-b border-yellow-600/30 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-stretch justify-between gap-2">
                        ...
                        {/* Scrolled mini brand */}
                        <Link to="/"
                            className={`flex items-center gap-2 font-bold text-sm flex-shrink-0 transition-all duration-300 ${
                                scrolled ? "opacity-100 py-2 mr-3 w-auto" : "opacity-0 w-0 overflow-hidden pointer-events-none"
                            }`}>
            ...
            {/* Spacer — compensates for the fixed header so page content starts below it.
                Uses two heights: full header (top bar + nav) when not scrolled,
                nav-only height when scrolled. This prevents any content jump. */}
            <div
                className="transition-all duration-300"
                style={{ height: scrolled ? "57px" : "117px" }}
            />
        </>
    );
}